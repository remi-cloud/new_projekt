...
export default function LiquidationHeatmapBar({
  ...
  heatmap: Heatmap | null | undefined
  entry?: number
  stop?: number
  tp1?: number
  tp2?: number
  ...
  const price = heatmap?.price ?? 0
  const range_low = heatmap?.range_low ?? 0
  const range_high = heatmap?.range_high ?? 0
  // Preview API sends bins only (1 column). Face loop needs ≥2 cols or the mesh is empty.
  const grid = useMemo(() => {
    if (columns && columns.length >= 2) return columns
    if (columns && columns.length === 1) return [columns[0], columns[0]]
    if (bins.length) {
      // Extrude single ridge into a short 3D strip so preview is visible before hydrate
      return [bins, bins, bins, bins]
    }
    return []
  }, [columns, bins])
  ...
  if (!heatmap) {
    return (
      <div className="heatmap-wrap hm3">
        <div className="hm3-empty">Brak danych mapy likwidacji dla tego instrumentu.</div>
      </div>
    )
  }
  ...
  if (!grid.length) {
    return (
      <div className="heatmap-wrap hm3">
        <div className="hm3-empty">Brak danych mapy — poczekaj na dociągnięcie 3D albo odśwież skan.</div>
      </div>
    )
  }
  ...
  return (
    <div className="heatmap-wrap hm3">
      ...
      <div className="heatmap-scale hm2-scale">
        <span>wcześniej</span>
        <span>
          zakres {fmt(range_low)} — {fmt(range_high)}
        </span>
        <span>teraz</span>
      </div>
    </div>
